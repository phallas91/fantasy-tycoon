...
func _draw_cities(cap: Vector2, cities: Array, _ci: int) -> void:
	...
	# Marker LOD alone cannot prevent long translated city names from colliding.
	# Keep a tiny screen-space layout registry and place every visible text chip
	# into the first free slot around its hub. This is deterministic (no jitter
	# between frames) and makes zoom inspection usable in every language.
	...
	for i in range(cities.size()):
		...
		if i == 0:
			...
			_label(_place_city_label(cp, nm, occupied_labels), nm, GOLD)
			occupied.append(cp)
			...
			if i == GameState.cities_unlocked or zoom >= 1.65:
				_label(_place_city_label(cp, nm, occupied_labels), nm, Color(0.80, 1.0, 1.0))
		else:
			var is_next := (i == GameState.cities_unlocked + 1)
			_locked_marker(cp, is_next)
			if is_next:
				_cost_chip(cp, _next_cost_str, occupied_labels)
...
func _chip_slot_free(rect: Rect2, occupied: Array[Rect2]) -> bool:
	for prior: Rect2 in occupied:
		if rect.grow(5.0).intersects(prior):
			return false
	return true

## Finds a stable collision-free position for a city-name pill. The returned
## point keeps the legacy `_label()` drawing contract (pill starts at y + 22).
func _place_city_label(p: Vector2, text: String, occupied: Array[Rect2]) -> Vector2:
	var tw := minf(_measure(text, 18), size.x - 32.0)
	var candidates: Array[Vector2] = [
		p,
		p + Vector2(0.0, 34.0),
		p + Vector2(0.0, -72.0),
		p + Vector2(tw * 0.5 + 38.0, -24.0),
		p + Vector2(-tw * 0.5 - 38.0, -24.0),
	]
	for raw: Vector2 in candidates:
		var q := Vector2(
			clampf(raw.x, tw * 0.5 + 16.0, size.x - tw * 0.5 - 16.0),
			clampf(raw.y, band_top - 16.0, band_bottom - 54.0)
		)
		var rect := Rect2(q.x - tw * 0.5 - 10.0, q.y + 22.0, tw + 20.0, 26.0)
		if _chip_slot_free(rect, occupied):
			occupied.append(rect)
			return q
	# Extremely dense inspection view: stay on-screen even when every preferred
	# slot is occupied. The earlier markers are more important and keep priority.
	var fallback := Vector2(
		clampf(p.x, tw * 0.5 + 16.0, size.x - tw * 0.5 - 16.0),
		clampf(p.y, band_top - 16.0, band_bottom - 54.0)
	)
	occupied.append(Rect2(fallback.x - tw * 0.5 - 10.0, fallback.y + 22.0, tw + 20.0, 26.0))
	return fallback
...
func _cost_chip(p: Vector2, cost: String, occupied: Array[Rect2]) -> void:
	if _font == null:
		return
	# lives ABOVE the marker (name pills live below) so lanes never collide
	var lw := 200.0
	var tw := minf(_measure(cost, 17), size.x - 64.0)
	var chip_y := p.y - 46.0
	if _hub_city2 != null:
		chip_y = p.y - (74.0 if zoom < 1.65 else 88.0)
	var cx := clampf(p.x, tw * 0.5 + 28.0, size.x - tw * 0.5 - 28.0)
	var candidates: Array[Vector2] = [
		Vector2(cx, chip_y), Vector2(cx, p.y + 30.0),
		Vector2(cx + tw * 0.5 + 38.0, p.y - 24.0),
		Vector2(cx - tw * 0.5 - 38.0, p.y - 24.0),
	]
	var pill := Rect2()
	for raw: Vector2 in candidates:
		var pcx := clampf(raw.x, tw * 0.5 + 28.0, size.x - tw * 0.5 - 28.0)
		var py := clampf(raw.y, band_top + 6.0, band_bottom - 32.0)
		var candidate := Rect2(pcx - tw * 0.5 - 27.0, py, tw + 45.0, 26.0)
		if _chip_slot_free(candidate, occupied):
			pill = candidate; cx = pcx; chip_y = py
			break
	if pill.size == Vector2.ZERO:
		chip_y = clampf(chip_y, band_top + 6.0, band_bottom - 32.0)
		pill = Rect2(cx - tw * 0.5 - 27.0, chip_y, tw + 45.0, 26.0)
	occupied.append(pill)
	...
	draw_string(_font, Vector2(cx - lw * 0.5 + 9.5, chip_y + 18.5), cost, HORIZONTAL_ALIGNMENT_CENTER, lw, 17, Color(SHADOW.r, SHADOW.g, SHADOW.b, 0.7))
	draw_string(_font, Vector2(cx - lw * 0.5 + 9.0, chip_y + 17.0), cost, HORIZONTAL_ALIGNMENT_CENTER, lw, 17, Color(GOLD.r, GOLD.g, GOLD.b, 0.95))
